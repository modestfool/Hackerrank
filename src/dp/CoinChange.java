...
public class CoinChange {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(numSolutions(coins,n));
    }

    static long numSolutions(int[] coins, int value)
    {
        if (value == 0)
            return 0;
        long [][] solutions = new long[value + 1][coins.length];
        ...
        for (int i = 1; i <= value; i++)
        {
            ...
            {
                long x, y;
                x = (i-coins[j] >= 0)? solutions[i - coins[j]][j]: 0;

                // Count of solutions excluding S[j]
                y = (j >= 1)? solutions[i][j-1]: 0;
...
//            System.out.println(Arrays.toString(a));
        return solutions[value][coins.length-1];
    }
}
